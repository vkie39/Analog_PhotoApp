...
class LocationSelectScreenState extends State<LocationSelectScreen>{
  ...
  static const CameraPosition _initialCamera = CameraPosition(
    target: LatLng(37.5665, 126.9780), // 서울시청 좌표 예시
    zoom: 14.0,
  );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GoogleMap(
        ...
        onMapCreated: (c) => _mapController = c,  // 지도 위젯이 만들어질 때 생성되는 컨트롤러를 _mapController로 사용
        myLocationButtonEnabled: true,            // 버튼 클릭시 내 위치로 카메라 이동
        myLocationEnabled: false,                 // 지도 위에 내 위치 표시
