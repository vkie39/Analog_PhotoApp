...
class RequestDetailScreen extends StatefulWidget {
  ...
  // => final String requestId;  firestore에서 실시간으로 detail페이지 받아올 때는 request자체가 아니라 requestId로 페이지 요청
  const RequestDetailScreen({super.key, required this.request,}); // => required this.requestId
...
class RequestDetailScreenState extends State<RequestDetailScreen> {
  ...
  bool isMarkedRequest = false;                                            // TODO : DB와 연동하여 좋아요를 누른 사용자이면 카운트하지 말고 처음부터 좋아요 표시가 되어있어야 함
  int markCount = 0;
